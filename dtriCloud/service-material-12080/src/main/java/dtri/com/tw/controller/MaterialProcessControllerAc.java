...
import dtri.com.tw.service.MaterialProcessService;
...
@RestController
/** 功能模組: 設置-製程對照表 */
public class MaterialProcessControllerAc extends AbstractControllerAc {
    ...
    @RequestMapping(value = { "/materialProcess/getSearch" }, method = {
    ...
    PackageBean getSearch(@RequestBody String jsonObject) {
        ...
        try {
            ...
            materialProcessService.getSearch(packageBean);
    ...
    @RequestMapping(value = { "/materialProcess/setModify" }, method = {
            RequestMethod.POST }, produces = "application/json;charset=UTF-8")
    PackageBean setModify(@RequestBody String jsonObject) {
        ...
        try {
            ...
            // Step2.執行=>服務
            loggerInf(funName + "[Start]", packageBean.getUserAccount());
            packageBean = materialProcessService.setModify(packageBean);
            loggerInf(funName + "[End]", packageBean.getUserAccount());
        } catch (JsonProcessingException e) {
            // StepX-1. 已知-故障回報
            e.printStackTrace();
    ...
    @RequestMapping(value = { "/materialProcess/setDelete" }, method = {
            RequestMethod.POST }, produces = "application/json;charset=UTF-8")
    PackageBean setDelete(@RequestBody String jsonObject) {
        ...
        try {
            ...
            // Step2.執行=>服務
            loggerInf(funName + "[Start]", packageBean.getUserAccount());
            packageBean = materialProcessService.setDelete(packageBean);
            loggerInf(funName + "[End]", packageBean.getUserAccount());
        } catch (JsonProcessingException e) {
            // StepX-1. 已知-故障回報
            e.printStackTrace();
