...
public class WarehouseAssignmentController extends AbstractController {
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/warehouse_assignment.basil.SS2" }, method = { RequestMethod.PUT })
	String modifyPassAll0(@RequestBody String jsonObject) {
		// 顯示方法
		String funName = new Object() {
		}.getClass().getEnclosingMethod().getName();
		sysFunction(funName);
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.setAssignmentModifyPassAll0(packageService.beanToJson(packageBean));
			loggerInf(funName + "[End]", loginUser().getUsername());
		} catch (Exception e) {
			// StepX-2. 未知-故障回報
			loggerWarn(eStktToSg(e), loginUser().getUsername());
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/warehouse_assignment.basil.SS4" }, method = { RequestMethod.PUT })
	String modifyMerge(@RequestBody String jsonObject) {
		// 顯示方法
		String funName = new Object() {
		}.getClass().getEnclosingMethod().getName();
		sysFunction(funName);
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.setAssignmentModifyMerge(packageService.beanToJson(packageBean));
			loggerInf(funName + "[End]", loginUser().getUsername());
		} catch (Exception e) {
			// StepX-2. 未知-故障回報
			loggerWarn(eStktToSg(e), loginUser().getUsername());
