...
@Controller
public class ScheduleProductionNotesController extends AbstractController {
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/schedule_production_notes.basil" }, method = {
	...
	String access(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.getProductionNotesSearch(packageService.beanToJson(packageBean));
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/schedule_production_notes.basil.AR" }, method = {
			RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/schedule_production_notes.basil.ARO" }, method = {
			RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	String searchOrder(@RequestBody String jsonObject) {
		// 顯示方法
		String funName = new Object() {
		}.getClass().getEnclosingMethod().getName();
		sysFunction(funName);
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.getProductionNotesSearchOrder(packageService.beanToJson(packageBean));
			loggerInf(funName + "[End]", loginUser().getUsername());
		} catch (Exception e) {
			// StepX-2. 未知-故障回報
			e.printStackTrace();
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/schedule_production_notes.basil.AU" }, method = { RequestMethod.PUT })
	String modify(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.setProductionNotesModify(packageService.beanToJson(packageBean));
			loggerInf(funName + "[End]", loginUser().getUsername());
		} catch (Exception e) {
			// StepX-2. 未知-故障回報
			e.printStackTrace();
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/schedule_production_notes.basil.AD" }, method = { RequestMethod.DELETE })
	String invalid(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.setProductionNotesInvalid(packageService.beanToJson(packageBean));
			loggerInf(funName + "[End]", loginUser().getUsername());
		} catch (Exception e) {
			// StepX-2. 未知-故障回報
			e.printStackTrace();
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/schedule_production_notes.basil.DD" }, method = { RequestMethod.DELETE })
