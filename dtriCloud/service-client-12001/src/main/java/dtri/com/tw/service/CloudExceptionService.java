...
public class CloudExceptionService extends Exception {
	 ...
	 * @param Language   國家語言?
	 */
	public CloudExceptionService(ErColor errorColor, ErCode errorCode, Lan language, String[] message) {
		...
		try {
			...
			if (message != null) {
				for (int i = 0; i < message.length; i++) {
					this.setErrorCodeMessage(this.getErrorCodeMessage().replaceAll("${" + i + "}", message[i]));
				}
			}
		} catch (Exception e) {
			// 未知-錯誤訊息(比對不到)
