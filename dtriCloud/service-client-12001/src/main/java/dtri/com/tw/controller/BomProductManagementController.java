...
public class BomProductManagementController extends AbstractController {
	...
	@ResponseBody
	@RequestMapping(value = { "/ajax/bom_product_management.basil.ART" }, method = {
			RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	String searchTest(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.getProductManagementSearchTest(packageService.beanToJson(packageBean));
			loggerInf(funName + "[End]", loginUser().getUsername());
