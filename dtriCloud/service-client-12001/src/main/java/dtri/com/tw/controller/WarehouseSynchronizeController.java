...
public class WarehouseSynchronizeController extends AbstractController {
	...
	String report(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.getSynchronizeReport(packageService.beanToJson(packageBean));
	...
	String add(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.setSynchronizeAdd(packageService.beanToJson(packageBean));
		} catch (Exception e) {
			// StepX-2. 未知-故障回報
			loggerWarn(e.toString());
			e.printStackTrace();
	...
	String modify(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.setSynchronizeModify(packageService.beanToJson(packageBean));
	...
	String invalid(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.setSynchronizeInvalid(packageService.beanToJson(packageBean));
	...
	String delete(@RequestBody String jsonObject) {
		...
		try {
			...
			// Step3.執行=>跨服->務執行
			packageBean = serviceFeign.setSynchronizeDetele(packageService.beanToJson(packageBean));
