...
public interface SystemGroupDao extends JpaRepository<SystemGroup, Long> {
	...
	@Query("SELECT c FROM SystemGroup c "//
			...
			+ "(:notsysstatus=4 or c.sysstatus != :notsysstatus ) and "// 排除-一般查詢(特定排除 Admin(3))/登入-排除(2資料作廢)/沒有要排除 (4)
			+ "(:sysstatus=4 or c.sysstatus = :sysstatus ) "// 指定-顯示/沒有要指定 (4)
			+ "order by c.sggid asc,c.sysheader desc,c.systemPermission.syssort asc")
	List<SystemGroup> findAllBySystemGroup(String sgname, //
			Long sggid, Long notsggid, //
			Integer sysstatus, Integer notsysstatus, //
			Pageable pageable);
	...
	ArrayList<SystemGroup> findAllByGroupTop1(String sgname);
